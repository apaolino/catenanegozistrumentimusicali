...
create table `tipo_evento` (
   `oid`  integer  not null,
   `nome_tipologia_evento`  varchar(255),
  primary key (`oid`)
);
...
create table `valutazione_negozio` (
   `recensione_oid`  integer not null,
   `negozio_oid`  integer not null,
  primary key (`recensione_oid`, `negozio_oid`)
);
alter table `valutazione_negozio`   add index fk_valutazione_negozio_recensi (`recensione_oid`), add constraint fk_valutazione_negozio_recensi foreign key (`recensione_oid`) references `recensione` (`oid`);
alter table `valutazione_negozio`   add index fk_valutazione_negozio_negozio (`negozio_oid`), add constraint fk_valutazione_negozio_negozio foreign key (`negozio_oid`) references `negozio` (`oid`);
...
create table `valutazione_prodotto` (
   `recensione_oid`  integer not null,
   `prodotto_oid`  integer not null,
  primary key (`recensione_oid`, `prodotto_oid`)
);
alter table `valutazione_prodotto`   add index fk_valutazione_prodotto_recens (`recensione_oid`), add constraint fk_valutazione_prodotto_recens foreign key (`recensione_oid`) references `recensione` (`oid`);
alter table `valutazione_prodotto`   add index fk_valutazione_prodotto_prodot (`prodotto_oid`), add constraint fk_valutazione_prodotto_prodot foreign key (`prodotto_oid`) references `prodotto` (`oid`);
...
create table `categoriaprodottocatalogobase` (
...
-- Indirizzo_Zona [rel35]
alter table `indirizzo`  add column  `zona_oid`  integer;
alter table `indirizzo`   add index fk_indirizzo_zona (`zona_oid`), add constraint fk_indirizzo_zona foreign key (`zona_oid`) references `zona` (`oid`);


-- Catena Negozi_Indirizzo [rel36]
alter table `catena_negozi`  add column  `indirizzo_oid`  integer;
alter table `catena_negozi`   add index fk_catena_negozi_indirizzo (`indirizzo_oid`), add constraint fk_catena_negozi_indirizzo foreign key (`indirizzo_oid`) references `indirizzo` (`oid`);
...
from  `prodotto` AL1 
               left outer join `prodottotorecensione_view` AL2 on AL1.`oid`=AL2.`s_oid`
