...
alter table `gestore_catena_utente_registra`   add index fk_gestore_catena_utente_reg_2 (`gestore_catena_oid`), add constraint fk_gestore_catena_utente_reg_2 foreign key (`gestore_catena_oid`) references `gestore_catena` (`utente_registrato_oid`);
alter table `gestore_catena_utente_registra`   add index fk_gestore_catena_utente_reg_3 (`utente_registrato_oid`), add constraint fk_gestore_catena_utente_reg_3 foreign key (`utente_registrato_oid`) references `utente_registrato` (`oid`);
...
-- EventoTipoEvento [rel37]
create table `eventotipoevento` (
   `evento_oid`  integer not null,
   `tipo_evento_oid`  integer not null,
  primary key (`evento_oid`, `tipo_evento_oid`)
);
alter table `eventotipoevento`   add index fk_eventotipoevento_evento (`evento_oid`), add constraint fk_eventotipoevento_evento foreign key (`evento_oid`) references `evento` (`oid`);
alter table `eventotipoevento`   add index fk_eventotipoevento_tipo_event (`tipo_evento_oid`), add constraint fk_eventotipoevento_tipo_event foreign key (`tipo_evento_oid`) references `tipo_evento` (`oid`);
...
-- CatalogoNegozio [rel43]
create table `catalogonegozio` (
   `catalogo_oid`  integer not null,
   `negozio_oid`  integer not null,
  primary key (`catalogo_oid`, `negozio_oid`)
);
alter table `catalogonegozio`   add index fk_catalogonegozio_catalogo (`catalogo_oid`), add constraint fk_catalogonegozio_catalogo foreign key (`catalogo_oid`) references `catalogo` (`oid`);
alter table `catalogonegozio`   add index fk_catalogonegozio_negozio (`negozio_oid`), add constraint fk_catalogonegozio_negozio foreign key (`negozio_oid`) references `negozio` (`oid`);
