...
alter table `specifico`  add column  `categoria_prodotto_oid`  integer;
alter table `specifico`   add index fk_specifico_categoria_prodott (`categoria_prodotto_oid`), add constraint fk_specifico_categoria_prodott foreign key (`categoria_prodotto_oid`) references `categoria_prodotto` (`oid`);
